...
static void SendToClient(const std::string& msg, MessageType type) {
  ...
  bool send{(PipeThreadStatus != ThreadStatus::NONE && PipeThreadStatus != ThreadStatus::WaitingForConnection)};
...
static void PipeThreadFunc() {
  ...
  while (!ShouldClose) {
    ...
    PipeHandle = CreateNamedPipe(PipeName.data(), PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE, 1, 0, 0, 0, NULL);
...
static uintptr_t mod_release() {
  ...
  if (PipeThreadStatus == ThreadStatus::WaitingForConnection) {
    ...
    HANDLE pipe = CreateFile(PipeName.data(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
    CloseHandle(pipe);
  }
